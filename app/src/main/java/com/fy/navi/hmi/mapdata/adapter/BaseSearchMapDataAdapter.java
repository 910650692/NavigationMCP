...
public abstract class BaseSearchMapDataAdapter<G, S extends BaseRecyclerHolder>
    ...
    /**
     * Get new GroupItem data for adapter.
     * 适配器显示新的数据。它必须被设置时，新的数据。
     * @param position
     * @return  返回一级级列表信息
     */
    public ProvDataInfo getGroupItem(final int position) {
        if (position >= 0 && position < mDataTrees.size()) {
            return (ProvDataInfo) mDataTrees.get(position).getGroupItem();
        } else {
            return null;
        }
    }
     ...
     * @param subItemIndex
     */
    public abstract void onSubItemBindViewHolder(final BaseRecyclerHolder holder,
                                                 final int groupItemIndex, final int subItemIndex);
     ...
     *                       一级条目点击事件
     */
    public abstract void onGroupItemClick(final Boolean isExpand, final G holder, final int groupItemIndex);
     ...
     *                       一级条目 按钮点击事件
     */
    public abstract void onGroupItemClick(final G holder, final int groupItemIndex);
        ...
        if (itemStatus.getViewType() == ItemStatus.VIEW_TYPE_GROUP_ITEM) {
            ...
            holder.itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(final View v) {
                    ...
                    final int areaType = ((ProvDataInfo) dt.getGroupItem()).getAreaType();
                    ...
                    if (areaType == 1) {
                        ...
                        onGroupItemClick((G) holder, groupItemIndex);
                    }
