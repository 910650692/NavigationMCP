...
public class RearScreenRouteHelper {
    public static final String TAG = RearScreenRouteHelper.class.getSimpleName();
    ...
    public static void refreshPathList() {
        ...
        if (mainPathIndex != NumberUtils.NUM_ERROR) {
            ...
            NaviAdapter.getInstance().updatePathInfo(MapType.REAR_SCREEN_MAP, pathInfos, mainPathIndex);
        } else {
    ...
    public static void showSelectPatch(final long newPathId) {
        ...
        if (!ConvertUtils.isEmpty(pathInfos) && null != selectPathInfo) {
            NaviAdapter.getInstance().updatePathInfo(MapType.REAR_SCREEN_MAP, pathInfos,
                    0);
        }
    }
}
