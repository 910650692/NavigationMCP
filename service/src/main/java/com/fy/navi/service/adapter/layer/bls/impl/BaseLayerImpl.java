...
public class BaseLayerImpl<S extends BaseStyleAdapter> extends PrepareLayerStyleInner implements ILayerClickObserver, ICarObserver, ILayerFocusChangeObserver, IBizRoadFacilityObserver {
    ...
    protected S createStyleAdapter(BizControlService bizService) {
        this.bizService = bizService;
        return null;
    }
        ...
        if (styleAdapter != null) {
            ...
            if (!TextUtils.isEmpty(layerItemJson)) {
                ...
                Logger.d(TAG, getClass().getSimpleName() +
                        ...
                        + "\n自定义 样式配置 : " + (TextUtils.isEmpty(styleJson) ? " null " : styleJson));
            }
        }
        ...
        if (LayerTextureManager.get().isNotValid(markerId)) {
            ...
            if (LayerTextureManager.get().isNotValid(markerId)) {
                ...
                if (layer.getMapView().addLayerTexture(layerTexture)) {
                    ...
                    Logger.d(TAG, getClass().getSimpleName() +
                            "\n  动态纹理添加成功  图层 :" + layer.getName() + ";图元业务类型 :" + item.getBusinessType() + ";图元类型 ：" + item.getItemType()
                            + "\n动态纹理ID:" + markerId
                            + "\n动态样式配置 : " + GsonUtils.toJson(styleInfo));
                } else {
                    Logger.e(TAG, getClass().getSimpleName() +
                            "\n  动态纹理添加失败  图层 :" + layer.getName() + ";图元业务类型 :" + item.getBusinessType() + ";图元类型 ：" + item.getItemType()
                            + "\n动态纹理ID:" + markerId
                            + "\n动态样式配置 : " + GsonUtils.toJson(styleInfo));
                }
            } else {
                Logger.d(TAG, getClass().getSimpleName() +
                        "\n   动态纹理添加 成功 图层 :" + layer.getName() + ";图元业务类型 :" + item.getBusinessType() + ";图元类型 ：" + item.getItemType()
                        + "\n 动态纹理池获取 ID:" + markerId
                        + "\n 动态样式配置 : " + GsonUtils.toJson(styleInfo));
            }
        } else {
            Logger.d(TAG, getClass().getSimpleName() +
                    "\n   默认纹理添加 成功 图层 :" + layer.getName() + ";图元业务类型 :" + item.getBusinessType() + ";图元类型 ：" + item.getItemType()
                    + "\n 默认纹理ID:" + markerId
                    + "\n 默认样式配置 : " + GsonUtils.toJson(styleInfo));
        }
        return markerId;
    }
    ...
    public List<ILayerAdapterCallBack> getCallBack(){
        return callBacks;
    }
