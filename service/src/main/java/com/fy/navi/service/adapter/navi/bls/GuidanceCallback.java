...
public class GuidanceCallback implements INaviObserver, ISoundPlayObserver {
    ...
    public GuidanceCallback(Hashtable<String, GuidanceObserver> guidanceObservers) {
    ...
    public void onUpdateNaviInfo(ArrayList<NaviInfo> naviInfoList) {
        NaviEtaInfo naviETAInfo = NaviDataFormatHelper.forMatNaviInfo(naviInfoList);
    ...
    public void onShowCrossImage(CrossImageInfo info) {
        CrossImageEntity naviImageInfo = NaviDataFormatHelper.forMatImageInfo(info);
        if (!ConvertUtils.isEmpty(mGuidanceObservers)) {
            for (GuidanceObserver guidanceObserver : mGuidanceObservers.values()) {
                if (guidanceObserver != null) {
    ...
    @Override
    public void onNaviStop(long traceId, int naviType) {
    ...
    public void onUpdateIntervalCameraDynamicInfo(ArrayList<NaviIntervalCameraDynamicInfo> cameraDynamicList) {
        if (!ConvertUtils.isEmpty(mGuidanceObservers)) {
            for (GuidanceObserver guidanceObserver : mGuidanceObservers.values()) {
                if (guidanceObserver != null) {
                    guidanceObserver.onNaviSpeedOverallInfo(NaviDataFormatHelper.forMatNaviSpeedCameraInfo(cameraDynamicList));
                }
            }
        }
    }
    ...
    public void onShowNaviCameraExt(ArrayList<NaviCameraExt> naviCameraList) {
        if (!ConvertUtils.isEmpty(mGuidanceObservers)) {
            for (GuidanceObserver guidanceObserver : mGuidanceObservers.values()) {
                if (guidanceObserver != null) {
                    guidanceObserver.onNaviCameraInfo(NaviDataFormatHelper.forMatNaviCameraInfo(naviCameraList));
                }
            }
        }
    }
    ...
    public void onUpdateTrafficLightCountdown(ArrayList<TrafficLightCountdown> list) {
        ...
        int num = 0;
        if (!ConvertUtils.isEmpty(list)) num = list.size();
        GeoPoint geoPoint = new GeoPoint();
        if (!ConvertUtils.isEmpty(list)) {
            TrafficLightCountdown lightCountdown = list.get(0);
            Coord2DDouble coord2DDouble = lightCountdown.position;
    ...
    public void onPlayTTS(SoundInfo pInfo) {
        if (!ConvertUtils.isEmpty(mGuidanceObservers)) {
            for (GuidanceObserver guidanceObserver : mGuidanceObservers.values()) {
                if (guidanceObserver != null) {
                    guidanceObserver.onPlayTTS(NaviDataFormatHelper.formatSoundInfo(pInfo));
                }
            }
        }
    }
    ...
    @Override
    public void onQueryAppointLanesInfo(long reqId, ArrayList<LaneInfo> laneInfo) {
        if (ConvertUtils.isEmpty(laneInfo)) return;
        if (ConvertUtils.isEmpty(mGuidanceObservers)) return;
        LaneInfo laneInfo1 = laneInfo.get(0);
        if (ConvertUtils.isEmpty(laneInfo1)) return;
        LaneInfoEntity laneInfoEntity = NaviDataFormatHelper.forMatLaneInfo(laneInfo1);
        for (GuidanceObserver guidanceObserver : mGuidanceObservers.values()) {
            if (guidanceObserver == null) continue;
    ...
    public void onShowSameDirectionMixForkInfo(final ArrayList<MixForkInfo> list) {
        ...
        List<NaviMixForkInfo> formaterMixForkList = NaviDataFormatHelper.formaterMixForkList(list);
        if (!ConvertUtils.isEmpty(mGuidanceObservers)) {
            for (GuidanceObserver guidanceObserver : mGuidanceObservers.values()) {
                if (guidanceObserver != null) {
