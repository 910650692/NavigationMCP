 ...
 * @date 2024/11/24
 */
public class NaviPackage implements GuidanceObserver {
    ...
    private Hashtable<String, IGuidanceObserver> guidanceObservers;
    ...
    private List<IsInForegroundCallback> mIsAppInForegroundCallbacks = new CopyOnWriteArrayList<>();

    private NaviPackage() {
        guidanceObservers = new Hashtable<>();
    ...
    public void registerObserver(String key, IGuidanceObserver guidanceObserver) {
        guidanceObservers.put(key, guidanceObserver);
    }

    /*移除GuidanceObserver回调，HMI层触发*/
    public void unregisterObserver(String key) {
        guidanceObservers.remove(key);
    }
    ...
    public void addNaviRecord(boolean isCompleted) {
        ...
        if (!isEmpty) {
            ...
            history.keyWord = endParam.getName();
            history.poiId = endParam.getPoiID();
            history.startPoiName = startParam.getName();
            history.endPoiName = endParam.getName();
            history.startPoint = startParam.getRealPos().toString();
            history.endPoint = endParam.getRealPos().toString();
            history.type = AutoMapConstant.SearchKeywordRecordKey.SEARCH_NAVI_RECORD_KEY;
    ...
    public void onNaviInfo(NaviEtaInfo naviETAInfo) {
        ...
        ThreadManager.getInstance().postUi(() -> {
            if (!ConvertUtils.isEmpty(guidanceObservers)) {
                for (IGuidanceObserver guidanceObserver : guidanceObservers.values()) {
    ...
    public void onPlayTTS(SoundInfoEntity pInfo) {
        ...
        ThreadManager.getInstance().postUi(() -> {
            if (NaviAdapter.getInstance().mCountDownLatch != null) {
                NaviAdapter.getInstance().setSoundInfoEntity(pInfo);
                NaviAdapter.getInstance().mCountDownLatch.countDown();
            }
            if (!mIsMute) {
                mSpeechAdapter.synthesize(pInfo.getText());
            }
        });
    }
