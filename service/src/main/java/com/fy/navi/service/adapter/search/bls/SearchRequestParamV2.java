...
import static com.autonavi.gbl.search.model.SearchSuggestionQueryType.General;
...
public class SearchRequestParamV2 {
     ...
     */
    public SearchSuggestionParam convertToSearchSuggestionParamV2(SearchRequestParameter searchRequestInfo) {
        SearchSuggestionParam param = new SearchSuggestionParam();
        param.keyword = searchRequestInfo.getKeyword();
        param.city = searchRequestInfo.getAdCode();
        param.queryType = General;
        param.type = 0;
        ...
        param.userLoc.lon = searchRequestInfo.getUserLoc().lon;
        param.userLoc.lat = searchRequestInfo.getUserLoc().lat;
     ...
     */
    public KeywordSearchTQueryParam convertToSearchKeywordParamV2(SearchRequestParameter searchRequestInfo) {
        KeywordSearchTQueryParam paramV2 = new KeywordSearchTQueryParam();
        ...
        paramV2.userLoc.lon = searchRequestInfo.getUserLoc().lon;
        paramV2.userLoc.lat = searchRequestInfo.getUserLoc().lat;
        paramV2.poiLoc.lon = searchRequestInfo.getUserLoc().lon;
        paramV2.poiLoc.lat = searchRequestInfo.getUserLoc().lat;
     ...
     */
    public SearchAggregateParam convertToSearchAggregateParamV2(SearchRequestParameter searchRequestInfo) {
        SearchAggregateParam paramV2 = new SearchAggregateParam();
     ...
     */
    public SearchEnrouteKeywordParam convertToSearchEnRouteKeywordParamV2(SearchRequestParameter searchRequestInfo) {
        SearchEnrouteKeywordParam paramV2 = new SearchEnrouteKeywordParam();
     ...
     */
    public SearchPoiDetailParam convertToSearchPoiDetailParamV2(SearchRequestParameter searchRequestInfo) {
        SearchPoiDetailParam param = new SearchPoiDetailParam();
    ...
    private void convertToSearchRetainParam(SearchRetainParamInfo searchRetainParamInfo, SearchRetainParam retainParam) {
        if (searchRetainParamInfo != null) {
            retainParam.keywordBizType = searchRetainParamInfo.getKeywordBizType();
            SearchTipsLocalInfo tipsLocalInfo = searchRetainParamInfo.getSearchTipsLocalInfo();
            ...
            for (SearchTipsCityLocalInfo cityLocalInfo : tipsLocalInfo.getOptionalCityList()) {
                SearchTipsCityInfo searchTipsCityInfo = new SearchTipsCityInfo();
     ...
     */
    public KeywordSearchIdqParam convertToKeywordSearchIdqParam(SearchRequestParameter searchRequestInfo) {
     ...
     */
    public SearchAlongWayParam convertToAlongWaySearchIdqParam(SearchRequestParameter searchRequestInfo) {
        SearchAlongWayParam param = new SearchAlongWayParam();
        // 路线上坐标点(至少两个)使用;分隔，如: 经度;纬度;经度;纬度，在线必传参数
        String flag = ";";
        String geoline = convertPointList2String(searchRequestInfo.getGeolinePointList(), flag);
        ...
        String routePoints = convertPointList2String(routePointList, flag);

        SearchAlongWayParam para = new SearchAlongWayParam();
        // 城市编码，离线搜索条件, 离线搜索必填
        para.adcode = searchRequestInfo.getAdCode();
        // 用户位置，经度,纬度 离线搜索用
        para.user_loc.lon = searchRequestInfo.getUserLoc().lon;
        para.user_loc.lat = searchRequestInfo.getUserLoc().lat;
     ...
     * @return SearchAlongWayParam
     */
    public KeywordSearchRqbxyParam convertToAroundSearchParam(SearchRequestParameter searchRequestInfo) {
        KeywordSearchRqbxyParam paramV2 = new KeywordSearchRqbxyParam();
        paramV2.keywords = searchRequestInfo.getKeyword();
        paramV2.pageParam.pageSize = DEFAULT_RESULT_MAX_COUNT;
        paramV2.pageParam.pageNum = searchRequestInfo.getPage();
        paramV2.queryType = searchRequestInfo.getQueryType();
        paramV2.userLoc.lon = searchRequestInfo.getUserLoc().lon;
        paramV2.userLoc.lat = searchRequestInfo.getUserLoc().lat;
        paramV2.poiLoc.lon = searchRequestInfo.getPoiLoc().lon;
        paramV2.poiLoc.lat = searchRequestInfo.getPoiLoc().lat;
        paramV2.geoObj = searchRequestInfo.getGeoobj();
        paramV2.city = String.valueOf(searchRequestInfo.getAdCode());
        paramV2.range = "5000";
        return paramV2;
     ...
     */
    public GNavigationEtaqueryRequestParam convertToGNavigationEtaqueryRequestParam(SearchRequestParameter requestParameter) {
        ...
        stStartPointData.lon = requestParameter.getUserLoc().lon;
        stStartPointData.lat = requestParameter.getUserLoc().lat;
        ...
        stEndPointData.lon = requestParameter.getPoiLoc().lon;
        stEndPointData.lat = requestParameter.getPoiLoc().lat;
        javaRequest.end.points.add(stEndPointData);
        return javaRequest;
    }

    private String convertPointList2String(@Nullable List<GeoPoint> pointList, @Nullable String flag) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            sb.append(geoPoint.getLon()).append(flag).append(geoPoint.getLat());
            if (i != size - 1) {
                sb.append(flag);
            }
        }
