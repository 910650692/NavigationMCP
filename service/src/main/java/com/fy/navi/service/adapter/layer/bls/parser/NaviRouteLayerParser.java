...
/**
 * @Description TODO 算路或引导时的路线颜色配置
 * @Author lvww
 * @date 2024/12/10
 */
public class NaviRouteLayerParser {
    ...
    public void getRouteLayerStyle(BaseLayer baseLayer, LayerItem layerItem,
                                   RouteLayerStyle routeLayerStyle,
                                   IPrepareLayerStyle prepareLayerStyle,
                                   @RouteLayerStyleType.RouteLayerStyleType1 int type,
                                   boolean isNightMode, MapTypeId mapTypeId) {
        this.mapTypeId = mapTypeId;
        // 是否为小地图
        boolean isEaglLine = type == RouteLayerStyleType.EaglEye_Normal || type == RouteLayerStyleType.EaglEye_HightLight
                || type == RouteLayerStyleType.EaglEye_Offline
                || type == RouteLayerStyleType.EaglEye_Offline_HightLight;

        // 是否为离线style
        boolean isOfflineStyle = type == RouteLayerStyleType.Main_Offline || type == RouteLayerStyleType.Main_Offline_HightLight
                || type == RouteLayerStyleType.EaglEye_Offline
                || type == RouteLayerStyleType.EaglEye_Offline_HightLight;
...
//            overlayParam.texPreMulAlpha = true;
    ...
    private long[] getPassedColor(@RouteLayerStyleType.RouteLayerStyleType1 int type, boolean isNightMode, boolean isHighLight) {
        ...
        switch (type) {
            ...
            case RouteLayerStyleType.Main_Offline_HightLight:
                if (isNightMode) {
                    if (!isHighLight) {
                        ...
                        passedColor[0] = 0xff5e5f61L;
                        passedColor[1] = 0xff484848L;
                        passedColor[2] = 0xffffffffL;
                    }
                } else {
                    if (!isHighLight) {
                        passedColor[0] = 0x00000000;
                        ...
                        passedColor[0] = 0xffadb2b6L;
                        passedColor[1] = 0xff8d8f91L;
                        passedColor[2] = 0xffffffffL;
                    }
                }
                break;
            ...
            case RouteLayerStyleType.EaglEye_Offline_HightLight:
                if (isNightMode) {
                    passedColor[0] = 0xff838383L;
                    passedColor[1] = 0xff484848L;
                    passedColor[2] = 0xffffffffL;
                } else {
                    passedColor[0] = 0xff717172L;
                    passedColor[1] = 0xff8d8f91L;
                    passedColor[2] = 0xffffffffL;
                }
                break;
    ...
    public boolean getLineNeedShowArrow(int lineType) {
        return switch (lineType) {
            case GlLineItemType.TYPE_MARKER_LINE_DOT -> false;
            case GlLineItemType.TYPE_MARKER_LINE -> true;
            case GlLineItemType.TYPE_MARKER_LINE_ARROW -> true;
            default -> false;
        };
    }
                ...
                if (!isOffline) {
                    ...
                    if (isNightMode) {
                        ...
                        if (!isHighLight) {
                            ...
                            Logger.d(TAG, "====RouteLayerParser map_lr_road_white_front textureType = {?}", textureType);
                            fillLineId = prepareLayerStyle.getMarkerId(baseLayer, layerItem,
                                    new ItemStyleInfo("", "", "map_lr_road_white_front", ""));
                            borderLineId = prepareLayerStyle.getMarkerId(baseLayer, layerItem,
                                    new ItemStyleInfo("", "", "map_lr_road_white_back", ""));
...
//                borderLineRatio = 0.296875f;
                ...
                fillLineColor = 0xffffffff;
                borderColor = 0xffffffff;
                Logger.d(TAG, "====RouteLayerParser MapRouteTextureArrow textureType = {?}", textureType);
                break;
            // 轮渡线
        ...
        roadParam.fillLineWidth = getAdapterLineOverlayWidth(fillLineWidth, fillLineRatio, true);

        roadParam.borderLineId = borderLineId;
        roadParam.borderLineWidth = getAdapterLineOverlayWidth(borderLineWidth, borderLineRatio, true);
        roadParam.fillLineColor = fillLineColor;
        roadParam.borderColor = borderColor;
        Logger.d(TAG, "====RouteLayerParser fillLineWidth = {?},  borderLineWidth = {?}", roadParam.fillLineWidth, roadParam.borderLineWidth);
        return roadParam;
    }
    ...
    public PolylineTextureInfo getRouteTextureInfo(int lineType, boolean isInNavi) {
        ...
        switch (lineType) {
            ...
            case GlLineItemType.TYPE_MARKER_LINE_COLOR: {
                ...
                x2 = 0.95f;
                y2 = 0.5f;
                glTexLen = ResourceUtils.Companion.getInstance().getDimensionPixelSize(R.dimen.dp_16);
                break;
            }
            case GlLineItemType.TYPE_MARKER_LINE: {
                x1 = 0.0f;
...
//                mX2 = 0.5f;
//                mY2 = 0.0f;
                glTexLen = ResourceUtils.Companion.getInstance().getDimensionPixelSize(R.dimen.dp_108);
                break;
            }
            //id >= 3003 && id< 3010
            case GlLineItemType.TYPE_MARKER_LINE_DOT: {
                ...
                x2 = 1.0f;
                y2 = 0.0f;
                glTexLen = ResourceUtils.Companion.getInstance().getDimensionPixelSize(R.dimen.dp_30);
                break;
            }
            //id >= 3003 && id< 3010
            case GlLineItemType.TYPE_MARKER_LINE_FERRY: {
            ...
            case GlLineItemType.TYPE_MARKER_LINE_RESTRICT: {
                ...
                x2 = 0.5f;
                y2 = 0.0f;
                glTexLen = ResourceUtils.Companion.getInstance().getDimensionPixelSize(R.dimen.dp_32);
                break;
            }
            default:
                break;
     ...
     */
    public int getAdapterLineOverlayWidth(int width, float ratio, boolean needAdjust) {
        float result = width / ratio;
        MapSurfaceViewSizeParams mapSurfaceParams = MapPackage.getInstance().getMapSurfaceParam(mapTypeId);
        if (needAdjust) {
            result = result * (mapSurfaceParams.densityDpi / 160.0f);
    ...
    static class OverlayParamBoolean {
